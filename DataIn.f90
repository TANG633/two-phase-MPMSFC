...
!------------------------------------------------------------------
    ...
    do b = 1, nb_body
       ...
       parEnd = bd%par_end
       !$OMP PARALLEL DO PRIVATE(pt,volume) SCHEDULE(STATIC)
       do p = parBegin, parEnd
          ...
          !do i = 1, nb_particle  
          !    pt1 => particle_list(i)
          !    if (ABS(pt%Xp(1) - pt1%Xp(1)) < epsilon(1.0_8)) then
          ...
          volume = (DCell**3) / 8       ! particle volume 8 material points per cell in 3D
          pt%Volu = volume              ! initial volume
          pt%Vol = volume
...
!------------------------------------------------------------------
    ...
    do ibody = 1, nb_body
        ...
        !$OMP PARALLEL DO REDUCTION(+:cmass, cke, cie) SCHEDULE(STATIC)
